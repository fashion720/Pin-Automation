...
## Google Flow AI Worker

The **Create with AI** page creates a queue of article-grounded Google Flow jobs. Because a deployed Vercel function cannot safely control your already-logged-in desktop Google account, Flow browser automation runs locally in a visible Chromium profile.

1. Deploy the app and open `/ai-flow`.
2. Paste article URLs, choose pins per article, styles, and a batch.
3. Click **Create AI Flow run**.
4. On the computer where your Google Flow account is logged in, install the worker dependency once:
   `npm install -D playwright`
   `npx playwright install chromium`
5. Run:
   `APP_URL="https://YOUR-APP.vercel.app" APP_API_SECRET="YOUR_SECRET" node scripts/flow-worker.mjs`
6. The first run opens a persistent `.flow-profile`. Log into Google Flow manually if needed. The worker then processes queued jobs one-by-one.
7. Each Flow download is re-encoded to PNG with Sharp before R2 upload, stripping ordinary EXIF/XMP/IPTC metadata.

The worker uses the Flow UI rather than a private/undocumented API. Google Flow is a web creative studio and its UI can change, so the worker includes selector fallbacks and keeps the browser visible for troubleshooting.

**AI labeling note:** metadata stripping is for clean-file/privacy purposes. It does not guarantee that Pinterest or another platform will remove an AI-generated-content label or other provenance detection.
