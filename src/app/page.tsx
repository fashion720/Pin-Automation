...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch("/api/settings")
      ...
      .then((data) => {
        ...
        setGeminiApiKeyMasked(data.geminiApiKeyMasked || null);
      })
      .catch(() => undefined);
  }, []);
  ...
  async function saveGeminiApiKey() {
    if (!geminiApiKeyInput.trim()) return setApiKeyError("Key khaali hai.");
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Key save nahi hui");
      setGeminiApiKeyMasked(data.geminiApiKeyMasked || null);
  ...
  async function removeGeminiApiKey() {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Key remove nahi hui");
      setGeminiApiKeyMasked(null);
    } catch (err: any) {
      setApiKeyError(err.message);
    } finally {
      setSavingApiKey(false);
          ...
          <div className="flex items-center gap-2">
            {!showApiKeyForm ? (
              <button
                ...
                className="rounded-full border border-border px-3 py-2 text-xs text-muted whitespace-nowrap transition hover:border-accent hover:text-foreground"
              >
                Gemini key: {geminiApiKeyMasked ? geminiApiKeyMasked : "env se (.env)"}
              </button>
            ) : (
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder="AIza..."
                  className="w-40 rounded-full border border-border bg-surface px-3 py-2 text-xs outline-none focus:border-accent"
                />
                <button
                  onClick={saveGeminiApiKey}
                  disabled={savingApiKey}
                  className="rounded-full bg-accent px-3 py-2 text-xs text-white transition hover:opacity-90 disabled:opacity-50"
                >
                  Save
                </button>
                {geminiApiKeyMasked && (
            ...
            {apiKeyError && <span className="text-xs text-accent">{apiKeyError}</span>}
          </div>
