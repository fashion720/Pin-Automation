...
import { composePin, loadArticleImages } from "@/lib/compose";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const pins: Pin[] = [];
    // Download/validate every image candidate once before choosing templates.
    // Each ScrapedImage is one logical article image; its src/candidates are
    // fallbacks for the same image, so a failed CDN URL never counts as a
    // missing article photo if another candidate works.
    const allImageCandidates = article.images.flatMap((image) => {
      const candidates = image.candidates?.length ? image.candidates : [image.src];
      return candidates.slice(0, 6);
    });
    const preloadedImages = await loadArticleImages(allImageCandidates, article.url);
    const usableArticleImages: ScrapedImage[] = article.images
      .map((image): ScrapedImage | null => {
        const candidates = image.candidates?.length ? image.candidates : [image.src];
        const usable = candidates.find((candidate) => preloadedImages.has(candidate));
        return usable ? { ...image, src: usable, candidates } : null;
      })
      .filter((image): image is ScrapedImage => image !== null);

    if (usableArticleImages.length === 0) {
      return NextResponse.json({ error: "Is article ki koi bhi image download nahi ho saki (source shayad hotlink-protected hai)" }, { status: 422 });
    }
    ...
    const nextImages = (count: number) => {
      const picked: string[] = [];
      let scanned = 0;
      while (picked.length < count && scanned < usableArticleImages.length) {
        const image = usableArticleImages[cursor % usableArticleImages.length];
        cursor++;
        scanned++;
        if (!image) break;
        const candidate = image.src;
    ...
    const generationPlan: Array<{
      ...
      imageUrls: string[];
      seedImage: ScrapedImage | undefined;
    }> = [];
    ...
    for (let index = 0; index < layoutIds.length; index++) {
      ...
      if (imageUrls.length < template.imageCount) {
        console.warn(`[generate] skipping ${template.name}: needs ${template.imageCount} distinct article images, found ${imageUrls.length}`);
        continue;
      }
      const seedUrl = imageUrls[0];
      const seedImage = usableArticleImages.find((image) => image.src === seedUrl);
      generationPlan.push({
        index,
        layoutId,
        template,
    ...
    for (const { index, layoutId, template, imageUrls } of generationPlan) {
      ...
      const pinBuffer = await composePin(
        template,
        imageUrls,
        overlay.overlayText,
        { ...styleOverrides, footerText: siteText },
        { referer: article.url, preloadedImages, candidateUrls: allImageCandidates }
      );
