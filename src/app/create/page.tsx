...
export default function CreatePost() {
  ...
  async function generateOneWithRetry(url: string): Promise<void> {
    ...
    const perAttemptTimeoutMs = 45000; // fail fast instead of hanging forever
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        if (err?.name === "AbortError") {
          if (attempt < maxAttempts) {
            setBulkSummary(`${url} timeout ho gaya (${perAttemptTimeoutMs / 1000}s) — retry kar raha hoon (attempt ${attempt}/${maxAttempts})…`);
            continue;
          }
          throw new Error(`${url}: request timeout ho gaya, server response nahi de raha`);
        }
        throw err;
      } finally {
      ...
      const isDailyQuota = /per\s*day|perday|free_tier_requests|requestsperday/i.test(errorMessage);
      const isRateLimited = res.status === 429 || /429|too many requests/i.test(errorMessage);

      if (isDailyQuota) {
        // Daily free-tier quota (e.g. 20 requests/day) won't reset in seconds —
        // retrying with a short backoff is pointless. Fail fast instead of hanging.
        throw new Error(`${url}: Gemini daily free-tier quota khatam ho gaya hai. Home page se ek dusra Gemini model select karo (e.g. gemini-3.1-flash-lite) aur dobara try karo.`);
      }
      ...
      throw new Error(errorMessage || "Generate fail ho gaya");
    }
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10">
      ...
      <div className="inline-flex rounded-full border border-border p-1 mb-8">
        ...
        <Link
          href="/ai-flow"
          className="text-sm px-4 py-1.5 rounded-full transition text-muted hover:text-foreground"
        >
          Create with AI
        </Link>
      </div>
